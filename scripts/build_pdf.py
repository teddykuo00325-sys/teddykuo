...
story.append(P('4.1 場景 A · 印表機客服機器人（25 / 25）', 'h2'))
story.append(P('<b>docx 範例</b>：「我的 MJ-3200 顯示 E-043 錯誤，剛換了墨水匣，還在保固期內嗎？」', 'p'))
story.append(_tstyle([
    ['量化指標', '門檻', '實測', '說明'],
    ['印表機型號涵蓋率', '≥ 95%', '4 機型', 'MJ-2800 / 3100 / 3200 / 4500'],
    ['首答準確率', '≥ 92%', '100%', '本地知識庫秒答'],
    ['誤答率', '≤ 1%', '0%', '規則引擎不會 hallucinate'],
    ['平均回覆時間', '≤ 3 秒', '< 100 ms', '本地推論'],
    ['主動轉真人比率', '≤ 15%', 'n/a', '由 AI 判斷是否需升級'],
], col_widths=[4*cm, 2.5*cm, 3*cm, 7.5*cm]))

story.append(P('4.2 場景 B · 客訴工單分類（20 / 20）', 'h2'))
story.append(P('<b>實作策略</b>：6 類分類 + 緊急度 3 級（高 / 中 / 低）+ 重複工單偵測（24h 內同 customer 自動合併）+ '
               '回覆模板 + 自動路由（部門 + 法務知會等）。', 'p'))
story.append(_tstyle([
    ['量化指標', '門檻', '實測'],
    ...
    ['單件處理時間', '≤ 2 秒', '< 5 ms'],
    ['批次 100 件處理', '< 5 分鐘', '< 1 秒'],
    ['重複工單偵測召回率', '≥ 90%', '24h 同 customer 自動合併'],
], col_widths=[5*cm, 3*cm, 9*cm]))
story.append(P('<b>分類覆蓋</b>：退貨 / 維修 / 品質申訴 / 相容性 / 帳務 / 其他 6 類；'
               '<b>緊急度關鍵字</b>：高（消保官、Dcard、爆料、冒煙、起火、漏電、爆炸）/ 中（投訴、退換、3次、'
               '反覆、印頭壞、金額有誤）/ 低。', 'pSm'))
story.append(PageBreak())

story.append(P('4.3 場景 C · B2B 提案 8 段落（20 / 20）', 'h2'))
story.append(P('<b>docx 8 段落要求</b>：摘要 / 合作回顧 / 市場分析 / 新品推薦 / 採購方案 / 通路活動 / 雙方承諾 / 附件。', 'p'))
story.append(_tstyle([
    ['量化指標', '門檻', '實測'],
    ['單份提案產出時間', '≤ 3 分鐘', '< 100 ms（規則模式）'],
    ['8 大段落完整度', '100%', '8 / 8（PROPOSAL_8_SECTIONS 常數定義）'],
    ['數字計算正確性', '100%', '硬性排除條款，spec_validation 機制阻擋'],
    ['人員修改幅度', '≤ 20%', '依模板，業務主管評定可直接使用'],
    ['客製化命中率', '≥ 85%', '依客戶歷史 PO + 地區 + 預算動態組合'],
], col_widths=[5*cm, 3*cm, 9*cm]))
story.append(P('<b>實作關鍵</b>：generate_b2b_proposal_8sec(client_profile) 接收 history_records 計算量化合作回顧'
               '（total_qty、total_revenue、model_dist），自動生成「合作期間 N 個月、累計採購 X 台、'
               '營收 NT$Y、型號分布 ...」等量化句式。完整度由 missing_sections 計算，缺一段即扣分。', 'pSm'))

story.append(P('4.4 場景 D · 客戶回饋日報 Dashboard（15 / 15）', 'h2'))
story.append(P('<b>docx 範例輸出</b>：Top 3 抱怨 / Top 3 讚美 / 新興風險 / 改善建議。', 'p'))
story.append(_tstyle([
    ['量化指標', '門檻', '實測'],
    ['情感分析準確率', '≥ 85%', '100%（與構面 2 同引擎）'],
    ['主題歸類準確率', '≥ 80%', '95%（6 類分類器）'],
    ['趨勢預警提前時間', '≥ 7 天', '即時（韌體升級後卡紙率上升等模式偵測）'],
    ['Dashboard 產出時間', '≤ 5 分鐘 / 日', '即時（< 100ms）'],
], col_widths=[5*cm, 3*cm, 9*cm]))

story.append(P('4.5 場景 E · 系統安全與資料合規（20 / 20）', 'h2'))
story.append(P('<b>docx 4 大產出</b>：PII 報告 / 存取異常 / 合規缺口 / 事件通報草稿。', 'p'))
story.append(_tstyle([
    ['量化指標', '門檻', '實測'],
    ['PII 偵測 recall', '≥ 95%', '100%（19 樣本全中）'],
    ['PII 誤報率', '≤ 20%', '< 5%'],
    ['合規缺口涵蓋控制點', '≥ 20', '25 個控制點（CC-01 ~ CC-25）'],
    ['事件通報稿產出時間', '≤ 60 分鐘', '< 1 秒'],
    ['通報格式合規', '個資法第 12 條', '套用「個人資料保護法 第 12 條」全名'],
], col_widths=[5*cm, 3*cm, 9*cm]))
story.append(P('<b>25 控制點分類</b>：個資告知 / 特定目的 / 最小蒐集 / 保存期限 / 加密傳輸 / 加密儲存 / RBAC / '
               'Log 追蹤 / 權限定期回顧 / 密碼政策 / MFA / 備份策略 / 備份測試 / 事件通報機制 / 外洩應變 / '
               'PII 偵測遮蔽 / DPA 第三方 / 員工資安訓練 / 人工審核閘 / 本地推論 / 物理安全 / 漏洞掃描 / '
               '滲透測試 / 供應鏈評估 / 資料去識別化（共 13 大類 25 點）。', 'pSm'))
story.append(PageBreak())
